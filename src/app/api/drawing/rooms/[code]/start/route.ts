...
import { z } from "zod";
import { loadDrawingRoom, serializeDrawingRoom, startDrawingRound } from "@/lib/drawingRoomState";
...
export async function POST(req: Request, { params }: { params: { code: string } }) {
  ...
  if (!requester?.isHost) return NextResponse.json({ error: "بس المضيف يبدأ اللعبة" }, { status: 403 });
  if (room.status !== "LOBBY" && room.status !== "FINISHED") {
    return NextResponse.json({ error: "الجولة شغّالة الحين" }, { status: 409 });
  }

  try {
    await startDrawingRound(room.id);
  } catch (err) {
