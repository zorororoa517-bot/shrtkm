...
import { serializeDrawingRoom } from "@/lib/drawingRoomState";
...
export async function POST(req: Request) {
  ...
  for (let attempt = 0; attempt < 5; attempt++) {
    try {
      const room = await prisma.drawingRoom.create({
        data: {
      ...
      return NextResponse.json(await serializeDrawingRoom(room.id, hostLogin), { status: 201 });
    } catch (err: unknown) {
