...
import { serializeFruitWarRoom } from "@/lib/fruitWarRoomState";
...
export async function POST(req: Request) {
  ...
  for (let attempt = 0; attempt < 5; attempt++) {
    try {
      const room = await prisma.fruitWarRoom.create({
        data: {
      ...
      return NextResponse.json(await serializeFruitWarRoom(room.id, hostLogin), { status: 201 });
    } catch (err: unknown) {
