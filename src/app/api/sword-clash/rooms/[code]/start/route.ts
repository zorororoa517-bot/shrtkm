...
import { z } from "zod";
import { loadSwordClashRoom, serializeSwordClashRoom, startSwordClashRound } from "@/lib/swordClashRoomState";
...
export async function POST(req: Request, { params }: { params: { code: string } }) {
  ...
  if (!requester?.isHost) return NextResponse.json({ error: "بس المضيف يبدأ المعركة" }, { status: 403 });
  if (room.status !== "LOBBY") return NextResponse.json({ error: "المعركة شغّالة الحين" }, { status: 409 });

  try {
    await startSwordClashRound(room.id);
  } catch (err) {
